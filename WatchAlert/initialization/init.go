...
func InitBasic() {
	...
	// 初始化JWT签名密钥
	global.StSignKey = []byte(config.GetSignKey())
	...
	// 启动告警工单事件监听器
	startAlertTicketEventListener(ctx)
...
// startAlertTicketEventListener 启动告警工单事件监听器
func startAlertTicketEventListener(ctx *ctx.Context) {
	listener := services.NewAlertTicketEventListener(ctx)
	listener.Start()
}
