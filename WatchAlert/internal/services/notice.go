...
func (n noticeService) Create(req interface{}) (interface{}, interface{}) {
	...
	hook := r.DefaultHook
	if r.NoticeType == "WeChatWork" && r.CorpId != "" && r.AgentId != "" && r.Secret != "" {
		hook = fmt.Sprintf("https://qyapi.weixin.qq.com/cgi-bin/message/send?corp_id=%s&corp_secret=%s&agentid=%s&touser=%s",
			r.CorpId, r.Secret, r.AgentId, r.ToUser)
	}
...
func (n noticeService) DeleteRecord(req interface{}) (interface{}, interface{}) {
	r := req.(*types.RequestNoticeQuery)
	err := n.ctx.DB.Notice().DeleteRecordByTenant(r.TenantId)
	if err != nil {
		return nil, err
	}
...
func (n noticeService) Test(req interface{}) (interface{}, interface{}) {
	...
	logc.Info(n.ctx.Ctx, fmt.Sprintf("[Notice Test] NoticeType: %s, DefaultHook: %s, Routes count: %d",
		r.NoticeType, r.DefaultHook, len(r.Routes)))
