...
type InterEventService interface {
	...
	ArchiveAlertEvent(req interface{}) (interface{}, interface{})
...
func (e eventService) ArchiveAlertEvent(req interface{}) (interface{}, interface{}) {
	r := req.(*types.RequestArchiveAlertEvent)
	...
	for _, fingerprint := range r.Fingerprints {
		go func(fp string) {
			defer wg.Done()
			event, err := e.ctx.Redis.Alert().GetEventFromCache(r.TenantId, r.FaultCenterId, fp)
			if err != nil {
				logc.Info(e.ctx.Ctx, fmt.Sprintf("获取告警事件失败: %v", err))
				return
			}

			e.ctx.Redis.Alert().RemoveAlertEvent(r.TenantId, r.FaultCenterId, fp)

			if err := process.RecordAlertHisEvent(e.ctx, event); err != nil {
				logc.Error(e.ctx.Ctx, fmt.Sprintf("记录告警历史失败: %v", err))
			}
		}(fingerprint)
	}
