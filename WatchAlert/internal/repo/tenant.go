...
type (
	...
	InterTenantRepo interface {
		...
		Search(keyword string, page, size int) (data []models.Tenant, total int64, err error)
...
func (tr TenantRepo) Search(keyword string, page, size int) (data []models.Tenant, total int64, err error) {
	offset := (page - 1) * size

	query := tr.db.Model(&models.Tenant{})
	if keyword != "" {
...
func (tr TenantRepo) AddTenantLinkedUsers(tenantId string, users []models.TenantUser, userRole string) error {
	...
	for _, u := range users {
		...
		// 同步更新members.role：如果用户只在单个租户中，则同步更新角色
		// 如果用户在多个租户中，members.role作为默认角色，不更新
		// 注意：需要在添加tenantId之前检查长度
		if !exist && len(userData.Tenants) == 0 && userData.Role != userRole {
