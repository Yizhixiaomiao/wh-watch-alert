...
type (
	...
	InterDutyRepo interface {
		...
		GetDuty(id string) (models.DutyManagement, error)
	}
)
...
// GetDuty 获取值班组（不依赖租户）
func (d DutyRepo) GetDuty(id string) (models.DutyManagement, error) {
	var data models.DutyManagement
	db := d.db.Model(&models.DutyManagement{})
	db.Where("id = ?", id)
	err := db.First(&data).Error
	...
	// 获取当前值班人员
	var dutySchedule models.DutySchedule
	d.DB().Model(models.DutySchedule{}).
		Where("duty_id = ? AND time = ?", id, time.Now().Format("2006-01-02")).
		Find(&dutySchedule)
	data.CurDutyUser = dutySchedule.Users
