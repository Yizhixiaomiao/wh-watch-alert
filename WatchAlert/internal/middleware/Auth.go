...
func Auth() gin.HandlerFunc {
	...
	return func(context *gin.Context) {
		...
		// 校验 Token
		code, userID, ok := IsTokenValid(ctx.DO(), tokenStr)
		if !ok {
			if code == 401 {
				response.TokenFail(context)
				context.Abort()
...
func IsTokenValid(ctx *ctx.Context, tokenStr string) (int64, string, bool) {
	// Bearer Token, 获取 Token 值
	if len(tokenStr) > len(tools.TokenType) && tokenStr[:len(tools.TokenType)] == tools.TokenType {
