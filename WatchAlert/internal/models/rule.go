...
type ClickHouseConfig struct {
	LogQL string `json:"logQL"`
	// ValueField 指定从查询结果中提取哪个字段作为告警判断的数值
	// 如果为空，则使用日志数量进行判断（兼容旧逻辑）
	ValueField string `json:"valueField,omitempty"`
	// LabelFields 指定哪些字段作为标签用于生成指纹
	// 如果为空，则使用 ruleId 生成指纹（兼容旧逻辑）
	LabelFields []string `json:"labelFields,omitempty"`
	// Annotations 告警详情模板，支持使用查询结果字段
	// 例如：域名 ${labels.domain} 的错误数为 ${labels.cnt_4xx}
