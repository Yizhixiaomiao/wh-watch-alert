...
func (m *ConsumeProbing) filterEvent(event models.ProbingEvent) bool {
	...
	case true:
		...
		if err != nil {
			logc.Error(m.ctx.Ctx, fmt.Sprintf("[Probe] remove probe event cache fail, err: %s", err.Error()))
		}
		return false
...
func (m *ConsumeProbing) sendAlert(alert models.ProbingEvent) {
	...
	// 获取值班用户
	dutyUsers := process.GetDutyUsers(m.ctx, noticeData)
	alert.DutyUser = strings.Join(dutyUsers, " ")
	...
	if err != nil {
		logc.Errorf(m.ctx.Ctx, err.Error())
		return
	}
}
