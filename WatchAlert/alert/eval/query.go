...
func logs(ctx *ctx.Context, datasourceId, datasourceType string, rule models.AlertRule) []string {
	...
	case provider.ClickHouseDsProviderName:
		...
		// 如果配置了 ValueField，则使用高级模式（从查询结果提取字段值）
		if rule.ClickHouseConfig.ValueField != "" {
			return clickhouseAdvancedMode(ctx, datasourceId, rule, cli)
		}
	...
	event := func() *models.AlertCurEvent {
		...
		case provider.ClickHouseDsProviderName:
			event.SearchQL = rule.ClickHouseConfig.LogQL
		}
...
// clickhouseAdvancedMode ClickHouse 高级告警模式
// 从查询结果中提取指定字段的值进行告警判断，类似 Metrics 的处理方式
func clickhouseAdvancedMode(ctx *ctx.Context, datasourceId string, rule models.AlertRule, cli interface{}) []string {
	var curFingerprints []string

	// 获取数据源信息
	datasourceObj, err := ctx.DB.Datasource().GetInstance(datasourceId)
	if err != nil {
		logc.Errorf(ctx.Ctx, "获取数据源失败: %v", err)
		return curFingerprints
	}

	// 创建高级模式的 ClickHouse 客户端
	advancedCli, err := provider.NewClickHouseAdvancedClient(ctx.Ctx, datasourceObj)
	if err != nil {
		logc.Errorf(ctx.Ctx, "创建 ClickHouse 高级模式客户端失败: %v", err)
		return curFingerprints
	}

	// 使用高级模式的 Provider 查询数据
	queryOptions := provider.LogQueryOptions{
		ClickHouse: provider.ClickHouse{
			Query: rule.ClickHouseConfig.LogQL,
		},
	...
	log, count, err := advancedCli.QueryAdvanced(queryOptions)
	if err != nil {
		logc.Error(ctx.Ctx, err.Error())
		return curFingerprints
	}

	externalLabels := advancedCli.GetExternalLabels()

	// 如果没有查询结果，直接返回
	if count == 0 || len(log.Message) == 0 {
		return curFingerprints
	}

	// 解析告警条件
	operator, expectedValue, err := tools.ProcessRuleExpr(rule.LogEvalCondition)
	if err != nil {
		logc.Errorf(ctx.Ctx, "解析告警条件失败: %v", err)
		return curFingerprints
	}

	// 遍历每条查询结果（类似 Metrics 的处理方式）
	for _, message := range log.Message {
		// 提取用于告警判断的数值字段
		var queryValue float64
		if fieldValue, exists := message[rule.ClickHouseConfig.ValueField]; exists {
			// 尝试转换为 float64
			...
			case string:
				// 尝试将字符串转换为数值
				if _, err := fmt.Sscanf(v, "%f", &queryValue); err != nil {
					logc.Errorf(ctx.Ctx, "字段 %s 的值 %v 无法转换为数值", rule.ClickHouseConfig.ValueField, v)
					continue
				}
			default:
				logc.Errorf(ctx.Ctx, "字段 %s 的值类型 %T 不支持", rule.ClickHouseConfig.ValueField, v)
				continue
			}
		} else {
			logc.Errorf(ctx.Ctx, "查询结果中未找到字段: %s", rule.ClickHouseConfig.ValueField)
			continue
		}

		// 构建标签用于生成指纹
		labels := make(map[string]interface{})
		if len(rule.ClickHouseConfig.LabelFields) > 0 {
			// 使用配置的标签字段
			for _, labelField := range rule.ClickHouseConfig.LabelFields {
				if labelValue, exists := message[labelField]; exists {
					labels[labelField] = labelValue
				}
			}
		} else {
			// 如果没有配置标签字段，使用所有字段
		...
		// 生成指纹
		fingerprint := log.GenerateFingerprintByLabels(labels)
		...
		// 获取初次触发值（类似 Metrics 的处理）
		cache, err := ctx.Redis.Alert().GetEventFromCache(rule.TenantId, rule.FaultCenterId, fingerprint)
		if err == nil && cache.Labels["first_value"] != nil {
			eventLabels["first_value"] = cache.Labels["first_value"]
		} else {
			eventLabels["first_value"] = queryValue
		}
		...
		event.Fingerprint = fingerprint
		event.SearchQL = rule.ClickHouseConfig.LogQL
		// ClickHouse 使用自定义的 Annotations 模板，如果没有则使用默认格式
		if rule.ClickHouseConfig.Annotations != "" {
			event.Annotations = tools.ParserVariables(rule.ClickHouseConfig.Annotations, tools.ConvertStructToMap(event))
		} else {
			event.Annotations = fmt.Sprintf("告警条件: %s %s %v, 当前值: %v", rule.ClickHouseConfig.ValueField, operator, expectedValue, queryValue)
		}
		...
		if process.EvalCondition(models.EvalCondition{
			...
			// 更新恢复时最新值
			cachedEvent, cacheErr := ctx.Redis.Alert().GetEventFromCache(event.TenantId, event.FaultCenterId, event.Fingerprint)
			if cacheErr == nil {
				if !cachedEvent.IsRecovered || cachedEvent.Status != models.StateRecovered {
					eventLabels["value"] = queryValue
					process.PushEventToFaultCenter(ctx, &event)
				}
			}
		}
