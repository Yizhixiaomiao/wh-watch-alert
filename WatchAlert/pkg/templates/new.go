...
func NewTemplate(ctx *ctx.Context, alert models.AlertCurEvent, notice models.AlertNotice) Template {
	...
	case "WeChatWork":
		return Template{CardContentMsg: wechatWorkTemplate(alert, noticeTmpl, notice.DefaultHook)}
	case "WeChatMiniProgram":
		return Template{CardContentMsg: wechatMiniProgramTemplate(alert, noticeTmpl)}
	case "PhoneCall":
