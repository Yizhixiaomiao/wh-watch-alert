...
func (noticeController noticeController) API(gin *gin.RouterGroup) {
	...
	)
	{
		...
		c.POST("noticeRecordDelete", noticeController.DeleteRecord)
	}
}
...
func (noticeController noticeController) DeleteRecord(ctx *gin.Context) {
	r := new(types.RequestNoticeQuery)
	BindJson(ctx, r)
	...
	Service(ctx, func() (interface{}, interface{}) {
		return services.NoticeService.DeleteRecord(r)
	})
}
