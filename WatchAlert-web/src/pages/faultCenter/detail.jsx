...
const handleGetSlo = async () => {
  ...
  // 使用 useMemo 确保 SLO 数据更新时重新计算
  const sloData = useMemo(() => ({
    mtta: computeAverageFromList("mtta"),
    mttr: computeAverageFromList("mttr")
  }), [sloChartData])  // 依赖 sloChartData，当它变化时重新计算

  const tagItems = [
    {
      key: "1",
      label: "活跃告警",
      children: <AlertCurrentEvent id={id} sloData={sloData} />,
    },
    {
      key: "2",
      label: "历史告警",
      children: <AlertHistoryEvent id={id} sloData={sloData} />,
    },
    {
      key: "3",
      label: "降噪配置",
