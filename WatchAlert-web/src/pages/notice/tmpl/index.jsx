...
export const NoticeTemplate = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record) => {
                ...
                }  else if (record.noticeType === 'WeChatWork') {
                    return (
                        <div style={{ display: 'flex' }}>
                            <WeChatIcon style={{ height: '25px', width: '25px' }} />
                            <div style={{ marginLeft: '5px', marginTop: '5px', fontSize: '12px' }}>企业微信应用</div>
                        </div>
                    );
    ...
    const handleList = async (skipCache = false) => {
        const res = await getNoticeTmplList({}, { skipCache });
        setList(res.data);
    };
    ...
    const handleDelete = async (record) => {
        ...
        clearCacheByUrl('/api/w8t/notice/tmpl')
        clearCacheByUrl('/api/w8t/notice/tmpl/list')
        handleList(true);
    };
    ...
    const onSearch = async (value) => {
        try {
            ...
            const res = await getNoticeTmplList(params, { skipCache: true });
            setList(res.data);
        } catch (error) {
            console.error(error);
        }
