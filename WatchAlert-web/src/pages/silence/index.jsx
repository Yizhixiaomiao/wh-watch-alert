...
export const Silences = (props) => {
    ...
    // 获取所有数据
    const handleList = async (index = pagination.index, size = pagination.size, skipCache = false) => {
        try {
            const params = {
                index: index,
                size: size,
            ...
            setLoading(true);
            const res = await getSilenceList(params, { skipCache });
            setLoading(false);
    ...
    const handleDelete = async (record) => {
        try {
            ...
            await handleList(pagination.index, pagination.size, true); // 重新加载当前页数据
        } catch (error) {
            console.error(error);
        }
    };
    ...
    const handleSearch = (value) => {
        ...
        handleList(1, pagination.size, true);
    };
