...
export const AlertCurrentEvent = (props) => {
    const { id, sloData } = props  // 接收 sloData 作为 props
            ...
            render: (_, record) => {
                const menu = (
                    <Menu>
                        ...
                        <Menu.Item onClick={() => handleArchive(record)}>
                            归档
                        </Menu.Item>
    ...
    const handleCurrentEventList = async (pageIndex, pageSize, skipCache = false) => {
        try {
            setLoading(true)
            const params = {
                faultCenterId: id,
            ...
            const res = await getCurEventList(params, { skipCache })
            if (res?.data?.list) {
                ...
                if (res.data.total > 0 && res.data.list.length === 0 && pageIndex > 1) {
                    ...
                    handleCurrentEventList(1, pageSize, skipCache)
                }
            }
        } catch (error) {
    ...
    const handleArchive = (record) => {
        Modal.confirm({
            title: "确认归档",
            content: `确定要归档告警 "${record.rule_name}" 吗？`,
            onOk: async () => {
                try {
                    await archiveAlert({
                    ...
                    clearCacheByUrl('/api/w8t/event/curEvent')
                    await handleCurrentEventList(currentPagination.pageIndex, currentPagination.pageSize, true)
                } catch (error) {
                    HandleApiError(error)
                }
            },
    ...
    const handleBatchArchive = () => {
        if (selectedRowKeys.length === 0) {
            message.warning('请先选择要归档的告警')
            return
        }

        Modal.confirm({
            title: "确认批量归档",
            content: `确定要归档选中的 ${selectedRowKeys.length} 个告警吗？`,
            onOk: async () => {
                try {
    ...
    async function handleExportClick() {
        ...
        exportAlarmRecordToHTML("活跃告警报表", event, [null, null], sloData || {});
    ...
    const fetchMetricData = async () => {
        try {
            // 只有 Prometheus 和 VictoriaMetrics 支持指标图表
            if (!selectedEvent.datasource_type ||
                (selectedEvent.datasource_type !== "Prometheus" &&
                 selectedEvent.datasource_type !== "VictoriaMetrics")) {
                return
            }
