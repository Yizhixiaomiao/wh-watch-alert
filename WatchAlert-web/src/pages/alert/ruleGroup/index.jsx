...
export const AlertRuleGroup = ({ }) => {
    ...
    const handleList = async (index, size, skipCache = false) => {
        try {
            const params = {
                index: index,
                size: size,
            }
            const res = await getRuleGroupList(params, { skipCache })
    ...
    const handleDelete = async (record) => {
        try {
            ...
            clearCacheByUrl('/api/w8t/ruleGroup')
            clearCacheByUrl('/api/w8t/ruleGroup/list')
            await handleList(pagination.index, pagination.size, true)
        } catch (error) {
            console.error(error)
        }
    }
    ...
    const onSearch = async (value) => {
        try {
            ...
            const res = await getRuleGroupList(params, { skipCache: true })
