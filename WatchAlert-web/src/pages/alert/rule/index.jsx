...
export const AlertRuleList = () => {
    ...
    useEffect(() => {
        handleList(selectedRuleGroupId, pagination.index, pagination.size, true)
    ...
    const handleList = async (id, index, size, skipCache = false) => {
        try {
            const params = {
                index: index,
                size: size,
            ...
            const res = await getRuleList(params, { skipCache })
    ...
    const handleDelete = async (ruleGroupId,ruleId) =>{
        try {
            ...
            clearCacheByUrl('/api/w8t/rule/ruleList')
            handleList(id, pagination.index, pagination.size, true)
        } catch (error) {
            HandleApiError(error)
        }
    }
