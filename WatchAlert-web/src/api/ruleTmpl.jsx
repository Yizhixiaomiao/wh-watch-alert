...
async function getRuleTmplList(params, options = {}) {
    try {
        const queryString = Object.keys(params)
            .map(key => params[key] !== undefined ? `${key}=${params[key]}` : '')
            .filter(Boolean)
            .join('&');
        const res = await http('get', `/api/w8t/ruleTmpl/ruleTmplList?${queryString}`, {}, options);
        return res;
    } catch (error) {
...
async function getRuleTmplGroupList(params, options = {}) {
    try {
        const res = await http('get', `/api/w8t/ruleTmplGroup/ruleTmplGroupList`, params, options);
        return res;
    } catch (error) {
        HandleApiError(error)
        return error
