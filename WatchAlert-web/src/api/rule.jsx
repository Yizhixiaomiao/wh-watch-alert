...
async function getRuleList(params, options = {}) {
    try {
        const res = await http('get', `/api/w8t/rule/ruleList`, params, options);
        return res;
    } catch (error) {
...
async function getRuleGroupList(params, options = {}) {
    try {
        const headers = {
            'TenantID': 'xxxxxxxxx'
        }
        const res = await http('get', '/api/w8t/ruleGroup/ruleGroupList', params, { ...options, headers });
        return res;
    } catch (error) {
        message.open({
            type: 'error',
