...
async function getNoticeList(params, options = {}) {
    try {
        const res = await http('get', '/api/w8t/notice/noticeList', params, options);
        return res;
    } catch (error) {
...
async function noticeRecordList(params, options = {}) {
    try {
        const res = await http('get', '/api/w8t/notice/noticeRecordList',params, options);
        return res;
    } catch (error) {
        HandleApiError(error)
        return error
...
async function deleteNoticeRecord(params) {
    try {
        const res = await http('post', '/api/w8t/notice/noticeRecordDelete', params);
        message.open({
            type: 'success',
            content: '通知记录清除成功',
        });
