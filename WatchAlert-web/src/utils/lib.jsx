...
export const HandleApiError = (error, prefixMessage = "错误") => {
    ...
    console.error('[HandleApiError] Error details:', error);

    // Check if it's an Axios response error and if response.data exists
    if (error.response && error.response.data) {
        console.log('[HandleApiError] Response data:', error.response.data);
        
        // 如果后端返回了失败状态（msg === "failed"）
