 ...
 * @param {Object} sloData - SLO 指标数据，包含 mtta 和 mttr
 * @returns {string} 生成的 HTML 字符串
 */
function generateHtmlContent(title, data, exportTimeRange, sloData = {}) {
    ...
    const formatDuration = (seconds) => {
        ...
        const s = Math.abs(Math.floor(Number(seconds)));
    ...
    const mttaValue = sloData.mtta !== undefined ? formatDuration(sloData.mtta) : "-";
    const mttrValue = sloData.mttr !== undefined ? formatDuration(sloData.mttr) : "-";

    // 格式化时间范围
    let timeRangeText = "全部时间";
    if (exportTimeRange && Array.isArray(exportTimeRange) && exportTimeRange[0] && exportTimeRange[1]) {
  ...
  `;
    ...
    <body>
      ...
      <div class="slo-container">
        <div class="slo-box">
          <div class="slo-icon">⏱️</div>
          <div class="slo-label">平均响应时间 (MTTA)</div>
          <div class="slo-value">${mttaValue}</div>
          <div class="slo-desc">Mean Time To Acknowledge</div>
        </div>
        <div class="slo-box">
          <div class="slo-icon">⚡</div>
          <div class="slo-label">平均修复时间 (MTTR)</div>
          <div class="slo-value">${mttrValue}</div>
          <div class="slo-desc">Mean Time To Repair</div>
        </div>
      </div>
 ...
 * @param {Object} sloData - SLO 指标数据，包含 mtta 和 mttr
 */
export async function exportAlarmRecordToHTML(title, data, exportTimeRange = [null, null], sloData = {}) {
    ...
    // 生成HTML内容
    const htmlContent = generateHtmlContent(title, data, exportTimeRange, sloData);
